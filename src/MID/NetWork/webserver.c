home > enc28j60-stm32 > websrv_help_functions.c
...
uint8_t find_key_val(char *str,char *strbuf, uint8_t maxlen,char *key)
{
        ...
        if (found==1){
                ...
                while(*str &&  *str!=' ' && *str!='\n' && *str!='&' && i<maxlen-1){ *strbuf="*str;" i++;="" str++;="" strbuf++;="" }="" ;="" return="" the="" length="" of="" value="" return(i);="" convert="" a="" single="" hex="" digit="" character="" to="" its="" integer="" unsigned="" char="" h2int(char="" c)="" {="" if="" (c="">= '0' && c <='9'){
...
void mk_net_str(char *resultstr,uint8_t *bytestr,uint8_t len,char separator,uint8_t base)
{
        ...
        uint8_t j=0;
        if (base == 10) {
            while(i<len){ itoa((int)bytestr[i],&resultstr[j],base);="" sprintf(&resultstr[j],="" "%d",="" (int)bytestr[i]);="" search="" end="" of="" str:="" while(resultstr[j]){j++;}="" resultstr[j]="separator;" j++;="" i++;="" }="" else="" if="" (base="=" 16)="" {="" while(i<len){="" "%x",="" j--;="" ;="" websrv_help_functions.c="" <="" pre="">
