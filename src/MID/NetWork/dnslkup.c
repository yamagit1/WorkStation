...
uint8_t udp_client_check_for_dns_answer(uint8_t *buf,uint16_t plen){
        ...
        */
        ...
        if (buf[UDP_DATA_P+i] & 0xc0){
                ...
                while(i<plen-udp_data_p-7){ i++;="" if="" (buf[udp_data_p+i]="=0){" break;="" }="" i="" is="" now="" pointing="" to="" the="" high="" octet="" of="" type="" field="" int="" numanswers="buf[UDP_DATA_P+7];" ansnum="0;" while(="" buf[udp_data_p+i+1]="" !="1" &&="" <="" )="" {="" +="buf[UDP_DATA_P+i+9]" 12;="" ansnum++;="" (="" ){="" dns_anserror="2;" not="" ipv4="" return(0);="" i+="10;" j="0;" while(j<4){="" dns_answerip[j]="buf[UDP_DATA_P+i+j];" j++;="" havednsanswer="1;" return(1);="" set="" dns="" server="" be="" used="" for="" lookups.="" defaults="" google="" called.="" void="" dnslkup_set_dnsip(uint8_t="" *dnsipaddr)="" uint8_t="" while(i<4){="" dnsip[i]="dnsipaddr[i];" #endif="" *="" end="" dnslkup.c="" pre="">


</plen-udp_data_p-7){></stdlib.h></string.h>
