home > enc28j60-stm32 > ip_arp_udp_tcp.c
...
void make_udp_reply_from_request(uint8_t *buf,char *data,uint16_t datalen,uint16_t port)
{
        ...
        // copy the data:
        while(i<datalen){ buf[udp_data_p+i]="data[i];" i++;="" }="" ck="checksum(&buf[IP_SRC_P]," 16="" +="" datalen,1);="" buf[udp_checksum_h_p]="ck">>8;
...
void send_udp(uint8_t *buf,char *data,uint16_t datalen,uint16_t sport, uint8_t *dip, uint16_t dport)
{
        ...
        // copy the data:
        i=0;
        while(i<datalen){ buf[udp_data_p+i]="data[i];" i++;="" }="" send_udp_transmit(buf,datalen);="" #endif="" udp_client="" #ifdef="" wol_client="" --------------------="" special="" code="" to="" make="" a="" wol="" packet="" (wake="" on="" lan)="" is="" udp="" the="" broadcast="" address="" and="" port="" 9.="" data="" part="" contains="" 6x="" ff="" followed="" by="" 16="" times="" mac="" of="" host="" wake-up="" void="" send_wol(uint8_t="" *buf,uint8_t="" *wolmac)="" {="" uint8_t="" i="0;" m="0;" pos="0;" uint16_t="" ck;="" while(i<6){="" buf[eth_dst_mac="" +i]="0xff;" buf[eth_src_mac="" buf[eth_type_h_p]="ETHTYPE_IP_H_V;" buf[eth_type_l_p]="ETHTYPE_IP_L_V;" fill_buf_p(&buf[ip_p],9,iphdr);="" buf[ip_totlen_l_p]="0x82;" buf[ip_proto_p]="IP_PROTO_UDP_V;" while(i<4){="" buf[ip_src_p+i]="ipaddr[i];" buf[ip_dst_p+i]="0xff;" fill_ip_hdr_checksum(buf);="" buf[udp_dst_port_h_p]="0;" buf[udp_dst_port_l_p]="0x9;" 9="" buf[udp_src_port_h_p]="10;" buf[udp_src_port_l_p]="0x42;" source="" does="" not="" matter="" buf[udp_len_h_p]="0;" buf[udp_len_l_p]="110;" fixed="" len="" zero="" checksum="" buf[udp_checksum_h_p]="0;" buf[udp_checksum_l_p]="0;" copy="" (102="" bytes):="" while="" (m<16){="" buf[pos]="wolmac[i];" pos++;="" m++;="" ck="checksum(&buf[IP_SRC_P]," 118,1);="">>8;
        buf[UDP_CHECKSUM_L_P]=ck& 0xff;
...
</datalen){></datalen){></stdio.h></stdlib.h></string.h>
